...
object AdaBoost extends App {
  ...
  val dataFileUri = this.getClass.getClassLoader.getResource("adaboost/car.data").toURI.getPath
  val data: AttributeDataset = read.csv(dataFileUri, attributes = attributes, response = Some((label, 6)))

  println("Sneak peek of the data:")
  println(data)

  val model = adaboost(data.x(), data.labels())
