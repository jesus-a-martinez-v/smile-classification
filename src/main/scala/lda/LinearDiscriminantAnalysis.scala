...
object LinearDiscriminantAnalysis extends App {
  ...
  val dataFileUri = this.getClass.getClassLoader.getResource("lda/balance-scale.data").toURI.getPath
  val data: AttributeDataset = read.csv(dataFileUri, attributes = attributes, response = Some((label, 0)))

  println("Sneak peek of the data:")
  println(data)

  val model = lda(data.x(), data.labels())
